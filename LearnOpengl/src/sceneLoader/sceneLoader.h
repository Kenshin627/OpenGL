...
public:
	SceneLoader();
	~SceneLoader();
	std::shared_ptr<Node> loadModel(const std::string& path, std::shared_ptr<X_Renderer> renderer);
...
private:
	std::shared_ptr<Node> processNode(aiNode* node, const aiScene* scene, std::shared_ptr<Node> parent, std::shared_ptr<X_Renderer> renderer);
	std::shared_ptr<Mesh> processMesh(aiMesh* mesh, const aiScene* scene, std::shared_ptr<X_Renderer> renderer);
